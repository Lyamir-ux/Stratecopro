-- 0085 - Suivi PPT : propositions du skill (schéma pppt-verif/1.1) à valider
--
-- Le skill pppt-verif décide désormais par défaut (classification, TVA,
-- années déduites, hypothèses, lissage…) et inscrit chaque décision dans
-- `propositions[]` avec un `statut_validation` (A_VALIDER, VALIDEE, REFUSEE,
-- MODIFIEE). Le dirigeant les tranche sur la revue (/ppt/rapports/:id) avant
-- la création du tableau ; ses décisions sont journalisées (ppt_corrections,
-- chemin `propositions[Pxx].statut_validation`) et conservées dans json_corrige.
--
-- Cette migration verrouille la règle côté base : ppt_valider_rapport refuse
-- de matérialiser les postes tant qu'une proposition reste « à valider »
-- (miroir de propositionsEnAttente côté app). Un JSON 1.0 (sans bloc) passe
-- comme avant. Le journal de validation compte les décisions.
...
begin
  ...
  -- propositions du skill (1.1) : toutes tranchées avant la création du tableau.
  ...
  -- journal de la copro en garde aussi la trace ci-dessous
